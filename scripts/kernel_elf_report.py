...
def build_report(kernel_elf: Path, linker_script: Path) -> dict[str, object]:
    ...
    issues = detected_issues(header, load_segments, symbol_address)
...
def detected_issues(header: ElfHeader, load_segments: list[ProgramHeader], symbol_address: int | None) -> list[str]:
    issues: list[str] = []
