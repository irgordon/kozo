...
def _required_texts() -> tuple[RequiredText, ...]:
    return (
        ...
        RequiredText("boot_doc_remaining_blocker", _BOOT_DOC_PATH, "Remaining blocker: `missing_limine_iso_tooling`.", "Boot doc must name remaining blocker"),
        RequiredText("blockers_doc_remaining_blocker", _BOOT_BLOCKERS_PATH, "The remaining blocker is `missing_limine_iso_tooling`.", "Boot blockers doc must name remaining blocker"),
    )
...
def _blocker_report_issue() -> BootImageIssue | None:
    ...
    if report.get("blocker_category") != "missing_limine_iso_tooling":
        return _issue("blocker_state_mismatch", "boot_blocker.blocker_category", "Boot blocker must be narrowed to missing_limine_iso_tooling")
    missing_components = report.get("missing_components")
    if not isinstance(missing_components, list) or "Limine ISO packaging command" not in missing_components:
        return _issue("blocker_state_mismatch", "boot_blocker.missing_components", "Boot blocker must require Limine ISO packaging tooling")
    return None
