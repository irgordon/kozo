...
| `v0.4.7` | Higher-Half Linker and Entry Transition | Attempt the focused linker/entry layout change needed to resolve `limine_lower_half_phdr` or record a narrower blocker. | Syscall behavior changes, serial fixes before kernel entry evidence, compatibility claims, runtime subsystem expansion. |
| `v0.4.8` | ABI and syscall maturity | Define ABI versioning, syscall expansion process, generated binding compatibility expectations, and regression evidence. | New syscall expansion without governed process. |
