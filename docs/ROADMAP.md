...
| `v0.5.0` | Fix Boot Smoke Marker Emission | Use CI evidence that entry and early serial initialization markers appear to focus on why `KOZO_BOOT_SMOKE_OK` is absent. | QEMU boot claim without `KOZO_BOOT_SMOKE_OK`, ABI or syscall behavior changes, broad runtime subsystem expansion. |
