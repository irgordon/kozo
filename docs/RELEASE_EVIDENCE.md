...
Passing v0.7.45 QEMU serial evidence proves only that QEMU launched the KOZO ISO, Limine loaded the KOZO kernel ELF, serial output was captured, the expected stack and memory evidence markers appeared, control crossed into the bounded Odin entry, the fixed Odin state probe completed, the exact success status returned, and the governed assembly continuation ran. It does not prove hardware trap execution, Linux compatibility, POSIX compatibility, userspace execution, process model behavior, VFS behavior, scheduler maturity, file descriptor behavior, production readiness, complete Odin runtime readiness, dynamic initialization, general stack readiness, general memory management, or syscall dispatch.
...
For v0.7.45 and later, release review must include `contracts/runtime_progression_entry_contract.v0.json`, `runtime_progression_entry_contract`, `runtime_progression_evidence`, the kernel ELF report, and QEMU metadata/logs. Contract and source validation prove only that the governed boundary is present. Executed progression and minimal Odin initialization are accepted only when CI captures `KOZO_RUNTIME_PROGRESS_ENTRY`, `KOZO_RUNTIME_INIT_OK`, and `KOZO_RUNTIME_RETURN_OK` in order and all validators pass.
...
v0.7.4 was accepted by the CI validator gate. Direct artifact content was not independently inspected because authenticated artifact access was unavailable; it must not be described as manually artifact-confirmed.
...
normalizes `.obj` or `.o` output to the caller's exact path. Every boot,
runtime-smoke, verification, and entrypoint-symbol build uses that helper.

The second hosted RC run confirmed that direct boot and QEMU preflight passed,
then correctly failed the clean-commit packaging gate because the preflight had
updated tracked evidence. Hosted CI now builds the commit-bound bundle before
...
For v0.8.1, release review must include
`contracts/cpu_extended_state_initialization_contract.v0.json`, both CPU-state
validators, the kernel ELF report, and QEMU metadata/logs. Acceptance requires
hosted evidence for CPU-state entry, successful readback-backed initialization,
the bounded SIMD probe, subsequent runtime progression, the existing capability
