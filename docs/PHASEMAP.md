...
| `v0.5.0` | Fix Boot Smoke Marker Emission | Use the CI-proven entry and serial initialization markers to focus on why `KOZO_BOOT_SMOKE_OK` is absent. | QEMU serial log/metadata, `_start` to `kernel_entry` handoff evidence, focused marker-emission tests, boot/runtime/release evidence docs. | QEMU boot remains unclaimed unless `KOZO_BOOT_SMOKE_OK` appears in captured QEMU serial output. |
