...
| `v0.4.7` | Higher-Half Linker and Entry Transition | Move toward a Limine-compatible higher-half kernel ELF layout if the structural audit does not reveal a blocking issue. | Focused linker script changes, entry/handoff evidence, regenerated kernel ELF report, QEMU smoke blocker evidence. | Current `limine_lower_half_phdr` blocker is resolved or narrowed without touching syscall behavior or overclaiming QEMU boot. |
| `v0.4.8` | ABI and syscall maturity | Stabilize current ABI/syscall governance and define expansion rules after the current boot-load blocker is classified. | ABI version policy, syscall expansion checklist, generated binding compatibility expectations, regression evidence for all governed syscalls. | Current syscall surface is frozen unless changed through governed process. |
