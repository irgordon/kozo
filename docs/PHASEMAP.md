...
| `v0.4.5` | ABI and syscall maturity | Stabilize current ABI/syscall governance and define expansion rules. | ABI version policy, syscall expansion checklist, generated binding compatibility expectations, regression evidence for all governed syscalls. | Current syscall surface is frozen unless changed through governed process. |
