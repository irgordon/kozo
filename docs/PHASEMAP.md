...
The next runtime phase should not expand claims automatically. It may implement memory initialization evidence only after this planning contract is validated in CI, and it must not replace halt behavior or claim runtime readiness without separate evidence.
