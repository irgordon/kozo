...
Full CI must install xorriso through apt, acquire the pinned Limine source release, verify the Limine source checksum, build Limine tooling, export `LIMINE_DIR`, `LIMINE`, and `XORRISO`, and run `scripts/build_boot_image.sh`.
...
QEMU smoke is not a required CI check while the active blocker is `missing_iso_generation_tooling`. `scripts/qemu_smoke.sh` is a release-local blocker review command until `artifacts/runtime/boot_image/kozo.iso` exists and CI support is explicitly added.
