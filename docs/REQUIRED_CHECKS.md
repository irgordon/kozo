...
If full CI fails after QEMU smoke runs, release review must treat the run as blocked even if uploaded artifacts appear promising. CI run `27894312430` produced passing QEMU serial smoke evidence, but that evidence remains a narrow smoke claim and does not prove Odin runtime execution, stack setup, memory initialization, syscall dispatch, hardware trap execution, compatibility, userspace behavior, or production readiness.
