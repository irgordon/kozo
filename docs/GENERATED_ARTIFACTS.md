...
| `artifacts/portability-comparison/cross-host-release-inputs.json` | required Linux, Windows, and macOS host-portability artifacts | `scripts/compare_portability_release_inputs.py` in `.github/workflows/portability.yml` | required cross-host release-input identity job and focused comparison tests | No | Run the required portability workflow |
