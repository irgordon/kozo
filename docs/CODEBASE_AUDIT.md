...
* current boot failure is classified as `limine_lower_half_phdr`
...
* release checklist and required-check wording has not yet been updated for the v0.4.5 `limine_lower_half_phdr` blocker
...
| AUDIT-002 | P1 | Boot layout | `linker/kernel.ld`, `kernel/arch/x86_64/boot.asm`, `kernel/main.odin`, `kernel/arch/x86_64/serial.odin`, `scripts/kernel_elf_report.py` | The current ELF entry and PT_LOAD virtual addresses are lower-half, while Limine rejects lower-half PHDRs. | `artifacts/runtime/kernel_elf_report.json` records `_start` and entry at `0x200000`, PT_LOAD virtual addresses starting at `0x200000`, and `limine_lower_half_phdr`. | Higher-half layout changes may break entry assumptions, physical load addresses, early serial access, and the kernel handoff path. | Make v0.4.7 a focused higher-half linker/entry transition. Require explicit physical `AT(...)` reasoning, regenerate ELF report, and inspect QEMU evidence before touching serial or syscall logic. | No |
| AUDIT-003 | P1 | Release gate wording | `docs/RELEASE_CHECKLIST.md`, `docs/REQUIRED_CHECKS.md` | Release/check wording still lists earlier QEMU blockers without `limine_lower_half_phdr`. | `rg` found blocker lists in release/check docs that include `limine_not_reached`, `kernel_not_loaded`, `kernel_entry_not_reached`, `serial_not_initialized`, `marker_not_emitted`, and `qemu_timeout` but not `limine_lower_half_phdr`. | Release review could miss the latest exact blocker even though validators and release evidence know it. | Patch release/check docs in a small follow-up or as part of the higher-half transition gate update. | No |
...
2. Update release/check docs to include `limine_lower_half_phdr` as an exact no-boot blocker.
