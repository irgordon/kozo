...
Development pre-alpha - Synchronous IPC & Hardened Multitasking

### Overview

Finalized the core architectural heartbeat of the KOZO microkernel. This version implements a high-performance Synchronous IPC mechanism based on direct register transfers, integrates critical hardware hardening (IST stacks, smart swapgs), and establishes a secure, zero-leak syscall environment with mandatory register cleansing.
...
- **Register Cleansing**: Implemented a mandatory scrub of callee-saved registers (`RBX`, `RBP`, `R8-R10`, `R12-R15`) in the syscall dispatcher to prevent info-leaks.
- **Interrupt Stack Table (IST)**: Configured dedicated stacks for `#DF` (Double Fault) and `#MC` (Machine Check) to survive stack exhaustion.
- **Smart swapgs**: Implemented conditional `swapgs` in `trap.S` based on the segment selector to prevent fatal double-swaps.
- **NXE Enforcement**: Enabled the No-Execute bit in the `EFER` MSR for hardware W^X protection.

#### Multitasking & TCB

- **Idle Thread**: Integrated a priority 255 background loop to keep the CPU stable during scheduling dead zones.
- **Unified Context Sync**: Enhanced `switchAway` to atomically update `TSS.RSP0`, syscall scratch area, and `CR3`.
- **TCB Safety**: Optimized `freeTCB` to nullify capability roots and zero sensitive state before reuse.

### Changed

- **Syscall Frame Architecture**: Migrated to a uniform frame layout that treats interrupts and syscalls as identical pointer types.
- **STAR MSR Configuration**: Hardened the `SYSCALL`/`SYSRET` segment alignment.
...
Development pre-alpha - Process Loading & CPIO Integration

### Overview

Established the bridge to user-space by implementing the CPIO initrd parser and the ELF64 binary loader. This version allows the kernel to search, parse, and execute independent services from memory-mapped archives.

### Added

- **CPIO Parser**: Implemented an alignment-aware "newc" format parser in `kernel/src/memory/cpio.zig`.
- **ELF64 Loader**: Developed a spec-compliant loader in `kernel/src/memory/elf.zig` that respects program headers and segment permissions.
- **Process Isolation**: Implemented `createAddressSpace` in the VMM to generate private page table hierarchies for new processes.
...
| ------- | ---------- | --------------------- | --------------------------- |
| 0.0.6   | 2026-03-01 | Development pre-alpha | IPC & Hardening             |
| 0.0.5   | 2026-03-01 | Development pre-alpha | Process Loading             |
