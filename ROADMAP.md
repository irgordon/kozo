...
| `v0.3.4` | Bootable image packaging | Produce a bootable Limine ISO or disk image from the current staged image root before retrying QEMU serial evidence. | QEMU boot claim until serial evidence is captured, Linux compatibility, userspace execution, runtime subsystem expansion. |
| `v0.3.5` | Security boundary foundation | Back pointer/null and authority-boundary rules with implementation evidence and negative tests. | Full formal verification, complete capability system. |
