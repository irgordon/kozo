...
| `v0.3.8` | QEMU Serial Smoke Evidence | Run the generated ISO under QEMU and validate serial evidence if ISO generation is available. | Linux compatibility, POSIX compatibility, process model, VFS, scheduler, ELF loading, file descriptors. |
| `v0.3.9` | Security boundary foundation | Back pointer/null and authority-boundary rules with implementation evidence and negative tests. | Full formal verification, complete capability system. |
