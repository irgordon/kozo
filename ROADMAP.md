...
| `v0.3.5` | Limine ISO tooling | Add the Limine bootloader artifacts and deterministic ISO tooling required to produce `artifacts/runtime/boot_image/kozo.iso`. | QEMU boot claim until serial evidence is captured, Linux compatibility, userspace execution, runtime subsystem expansion. |
| `v0.3.6` | Security boundary foundation | Back pointer/null and authority-boundary rules with implementation evidence and negative tests. | Full formal verification, complete capability system. |
