...
| `v0.3.2` | Boot Image Skeleton | Add linker script, Limine configuration, minimal image packaging, and QEMU invocation scaffolding without claiming boot success. | Compatibility claims, userspace execution, production readiness. |
| `v0.3.3` | QEMU serial smoke evidence | Capture and validate QEMU serial smoke output from the kernel entry path if the image skeleton is feasible. | Linux compatibility, POSIX compatibility, process model, VFS, scheduler, ELF loading, file descriptors. |
| `v0.3.4` | Security boundary foundation | Back pointer/null and authority-boundary rules with implementation evidence and negative tests. | Full formal verification, complete capability system. |
