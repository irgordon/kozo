...
var cap_lock = spinlock.SpinLock.init();
...
/// SYS_CAP_CREATE: Create capability from untyped (alias for retype)
pub fn sys_create(untyped: usize, obj_type: abi.kozo_cap_type_t, slot: usize) isize {
    return sys_retype(untyped, obj_type, slot, 0);
}

/// SYS_CAP_DELETE: Remove capability from CNode (don't free memory)
pub fn sys_delete(slot: usize) isize {
    const irq_state = cap_lock.lock();
    defer cap_lock.unlock(irq_state);
    
    const current = thread.getCurrent() orelse return -2;
    const target_slot = current.root_cnode.get(slot) catch return -2;
    
    if (target_slot.cap_type == .CAP_NULL) return -2;
    target_slot.* = NULL_SLOT;
...
pub const SyscallTable = struct {
    pub fn dispatch(num: usize, arg1: usize, arg2: usize, arg3: usize, arg4: usize) isize {
        return switch (num) {
            ...
            abi.SYS_CAP_CREATE => sys_create(arg1, @enumFromInt(arg2), arg3),
            abi.SYS_CAP_DELETE => sys_delete(arg1),
