...
| `v0.3.5` | Limine ISO tooling | Add Limine bootloader artifacts and deterministic ISO tooling needed to produce `artifacts/runtime/boot_image/kozo.iso`. | Limine bootloader artifacts, deterministic image packaging command, ISO artifact or narrower tooling blocker. | A bootable image artifact exists or a narrower tooling blocker is recorded without broad runtime or compatibility claims. |
| `v0.3.6` | Security boundary foundation | Convert security model from policy-only to minimal implementation-backed checks. | Pointer/null boundary enforcement evidence, capability/handle placeholder policy or initial implementation, fault containment expectations, negative tests for unsafe boundary cases. | Security boundary claims are backed by implementation evidence and negative tests. |
