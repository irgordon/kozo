...
| `v0.3.2` | Boot Image Skeleton | Add the minimum Limine image skeleton without claiming boot success until serial output is captured. | linker script, Limine config, minimal ISO or image packaging script, QEMU invocation script if feasible. | A boot image skeleton exists and any QEMU claim remains blocked until serial evidence is captured and validated. |
| `v0.3.3` | QEMU serial smoke evidence | Capture and validate bounded QEMU serial output from the kernel entry path if the boot image skeleton is feasible. | `artifacts/runtime/qemu_smoke.log`, QEMU smoke command, runtime evidence validator for QEMU smoke. | QEMU serial evidence exists or a narrower blocker is recorded without broad runtime or compatibility claims. |
| `v0.3.4` | Security boundary foundation | Convert security model from policy-only to minimal implementation-backed checks. | Pointer/null boundary enforcement evidence, capability/handle placeholder policy or initial implementation, fault containment expectations, negative tests for unsafe boundary cases. | Security boundary claims are backed by implementation evidence and negative tests. |
