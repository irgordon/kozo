...
| `v0.3.4` | Bootable image packaging | Produce a bootable Limine ISO or disk image from the current boot image skeleton. | Limine bootloader artifacts, deterministic image packaging, ISO/disk artifact, updated blocker state. | A bootable image artifact exists or a narrower packaging/tooling blocker is recorded without broad runtime or compatibility claims. |
| `v0.3.5` | Security boundary foundation | Convert security model from policy-only to minimal implementation-backed checks. | Pointer/null boundary enforcement evidence, capability/handle placeholder policy or initial implementation, fault containment expectations, negative tests for unsafe boundary cases. | Security boundary claims are backed by implementation evidence and negative tests. |
